...
1. Let users annotate documents with document anchors and optional paragraph/text-selection anchors.
...
- Center pane: document reader (full document)
- Right sidebar: unified context feed (`All`, `Notes`, `Provocation`)
- No persistent left pane in reader mode; document navigation is exposed through the top-bar `Documents` control and the Documents drawer.
...
2. User opens a document and navigates between documents.
3. User writes notes attached to that document, paragraph, or text selection.
4. User explicitly requests a provocation for:
   - the current document while reading, or
...
## 7.2 Ingestion and document structure
...
- Store each document as a single continuous section:
  - heading `Document`
  - `anchor_key = document#1`
- Preserve source order for document text; do not use fuzzy/AI segmentation in v0.
...
- Notes must attach to the document section and remain anchored across app restarts.
...
- If native `.pdf` rendering is unavailable, show fallback guidance while keeping document text and notes available.
...
- `.pdf` selections must map deterministically to normalized active-revision document-text offsets before note creation.
...
- `.pdf` renderer replacement must preserve zoom/scroll UX parity, meet document-navigation latency targets, and comply with Electron/CSP security baseline constraints.
- Create/edit/delete notes attached to a document, with optional paragraph/text-selection metadata.
...
- Provocations accumulate per document and are shown in the unified feed ordered by document appearance
...
- Build AI context deterministically from the active document only (no adjacent sections).
...
- Re-import/re-index executes atomically in one transaction:
  ...
  2. store document content for new revision
...
- Unmatched notes are marked unassigned by setting `notes.section_id` to `NULL` and creating an open `note_reassignment_queue` row with old document metadata.
- Reassignment resolves the queue row and sets `notes.section_id` to the document section in `documents.current_revision_id`.
...
- `sections`
  - `id`, `document_id`, `revision_id`, `anchor_key`, `heading`, `order_index`, `text`, `page_start`, `page_end` (single row per document revision)
- `notes`
...
- Target interactions:
  - open indexed document: near-instant local response
...
- Benchmark protocol:
  - v0 blocking smoke gate:
    - 20 warm-cache document navigations, report p50.
    - 10 provocation calls, report p50.
    - pass target: document navigation p50 `<200ms`; AI generation p50 `<8s`.
  ...
  - Hardening benchmark suite (non-blocking for initial hobby v0):
    ...
    - Document navigation benchmark: warm-cache run, 200 navigations, report p50/p90 from document-select to rendered document.
...
2. User can navigate document-by-document and add notes (including optional paragraph/text-selection anchors) that remain attached after restart.
3. User can request provocations for documents, notes, and selected text on demand.
...
16. Document structure and anchor generation behavior follows deterministic rules in section 7.2.
...
- Workspace creation
- File import + document ingestion
...
- Deterministic context assembly from active document

Phase 3: Thought scaffolding

- Section notes
- On-demand provocations (document + note + selected-text target)
